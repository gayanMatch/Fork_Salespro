...
def _load_model(ckpt_path, device, use_small=False, model_type="text"):
    ...
    for k, v in list(state_dict.items()):
        ...
        if k.startswith(unwanted_prefix):
            key_name = k[len(unwanted_prefix):]
        if model_type == 'text' or model_type == 'coarse':
            key_name = key_name.replace('transformer.h', 'transformer_h.layers')
        if k.startswith(unwanted_prefix):
            state_dict[key_name] = weights
...
def codec_decode(fine_tokens):
    ...
    out = None
    # print("Encode: ", time.perf_counter())
    with torch.cuda.amp.autocast(enabled=False):
        emb = model.quantizer.decode(arr)
        out = model.decoder(emb)
    # print("Encode Ended: ", time.perf_counter())
    # out = get_result(model, arr)
    ...
    return audio_arr
# @torch.jit
# def get_result(model, arr):
#     emb = model.quantizer.decode(arr)
#     return model.decoder(emb)
